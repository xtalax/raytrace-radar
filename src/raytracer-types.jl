...
mutable struct Point<:Geometry
    ...
    Point(x::Number,y::Number,z::Number) = new(x,y,z)
end
...
function buildTriangle(Points::Vector{Point})
...
mutable struct Surface <: Geometry
    ...
    extent::Float64
    RefrIndex::Float64
    function Surface(nodeCoords,triIndices,nTriangles,scale=1.0,RefrIndex=1.0,origin = [0.0,0.0,0.0])
        Triangles::Array{Triangle,1} = []
        Edges::Array{Edge,1} = []
        Nodes::Array{Node,1}= []
        NodeList::Dict = Dict([])
...
mutable struct Ray <: Beam#The Rays object
    ...
    tof::Float64 #time of flight of the Ray
    ampS::Float64
    ampP::Float64
    ϕs::Complex{Float64}
    ϕp::Complex{Float64}
    c::Float64
    function Ray(origin, direction, TEBasis, parent, tof=0.0, ampS = 1/sqrt(2), ampP = 1/sqrt(2), ϕs =1.0+0.0im, ϕp=1.0+0.0im,  c=c₀)


          new(origin, normalize(direction), TEBasis ,parent, tof, ampS, ampP, ϕs, ϕp,  c)
...
mutable struct Bundle
    Rays::Vector{Ray}
    N::Int64
    function Bundle(origin = Point(0.0,0.0,0.0)::Point, direction=Point([0.0,0.0,1.0])::Point, φ = 0.0, Nx=60::Int, Ny=60::Int, shape = "square"::String, X=0.5, Y=0.5)
        ...
        Origins = reshape(Origins, (N, ))
        rays = Ray.(Origins, (direction for i in 1:N), (TEBasis for i in 1:N), (Source(1.0,true) for i in 1:N))
